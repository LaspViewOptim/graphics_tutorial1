...
impl<'a> State<'a> {
    ...
    pub async fn new(window: &'a Window) -> State<'a> {
        ...
        let sphere_instances: Vec<Instance> = Vec::new();
        // initiate cylinder for bonds
        let cylinder = Cylinder::new(1.0, 1.0, 16);
        let cylinder_instances: Vec<Instance> = Vec::new();
    ...
    pub async fn load_structure(&mut self, block: &arc_parser::modules::structures::StructureBlock) {
                                ...
                                let future = async {
                                    match arc_parser::parser::parser::read_file(path.to_str().unwrap_or_default(), true) {
                                        Ok(Some(blocks)) if !blocks.is_empty() => {
                                            state.load_structure(&blocks[0]).await;
                                        }
                                        Err(e) => {
...
async fn create_empty_state<'a>(window: &'a Window) -> State<'a> {
    ...
    state.load_structure(&block).await;
    state
}